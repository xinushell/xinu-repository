...
beautiful.init("/home/neichus/.config/awesome/themes/theme.lua")
end
...
for s = 1, screen.count() do
    ...
    -- We need one layoutbox per screen.(icon of layouts)
    ...
    -- Add widgets to the wibox - order matters(all the elements of the panel)
    mywibox[s].widgets = {
        {
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "x",
              function ()
                  awful.prompt.run({ prompt = "Run in urxvt: " },
                  mypromptbox[mouse.screen].widget,
		  function (host) awful.util.spawn( terminal .. " -e " .. host) end)
              end),
